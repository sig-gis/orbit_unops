...
def run_11_03_01(
    ...
    **kwargs: Any,
    ...
    if step == 1:
        # =====================================================================
        # STEP 1: Train RF, evaluate metrics, and export COG image
        # =====================================================================
        ...
        # Prepare and optionally export prediction stats
        metrics_for_export = {
        ...
        # Mandatory image export to fuel Step 2
        ...
        result["gcs_image_url"] = f"gs://{gcs_bucket}/{geotiff_file_name_prefix}.tif"
        result["next_step"] = 2
        
    elif step == 2:
        # =====================================================================
        # STEP 2: Load COG from GCS and calculate statistics
        # =====================================================================
        if not previous_result or "gcs_image_url" not in previous_result:
            raise ValueError("Step 2 requires 'gcs_image_url' to be provided in previous_result.")
        ...
        try:
            ...
            blobs = list(bucket.list_blobs(prefix=geotiff_file_name_prefix))
            tif_uris = [f"gs://{gcs_bucket}/{b.name}" for b in blobs if b.name.endswith(".tif")]
            
            if not tif_uris:
                raise ValueError(f"No .tif files found for prefix {geotiff_file_name_prefix}")
            ...
            output_image = output_image.rename(year_names).toByte()
        except Exception as e:
            print(f"Warning: Failed to load tiled images via GCS list, falling back to single URL: {e}")
            output_image = ee.Image.loadGeoTIFF(previous_result["gcs_image_url"]).rename(year_names).toByte()
    ...
    try:
        url = f"/cog/tiles/WebMercatorQuad/{{z}}/{{x}}/{{y}}?url=gs://{gcs_bucket}/{result.get('geotiff_file_name_prefix', geotiff_file_name_prefix)}.tif&bidx=1&nodata=0&colormap=%7B%221%22:%22%23FF5722FF%22%7D"
        result["layers"] = [
            {
                "id": "urban_extent",
